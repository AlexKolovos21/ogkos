...
async function optionalGrokPwa(): Promise<Plugin[]> {
  try {
    const mod = (await import("./scripts/grok-pwa-plugin.mjs")) as { grokPwaPlugin: () => Plugin };
...
export default defineConfig(async ({ command, isPreview }) => ({
  ...
  plugins: [
    ...
    authPopupPlugin(),
    appEnvPlugin(),
    ...(await optionalGrokPwa()),
    tailwindcss(),
    tanstackStart(),
    ...(command === "build" || isPreview
      ? [nitro({ preset: "vercel", serverDir: existsSync("./server") ? "./server" : false })]
      : []),
    viteReact(),
  ],
}));
